...
class Sampler(StableDiffusionPipeline):
    ...
    def edit(
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps[-start_time:])):
            with record_function("denoise_step"):
                ...
                for ri in range(repeat):
                    ...
                    with torch.no_grad():
                        with record_function("denoise_unet"):
                    ...
                    if (eta_rd > 0 or eta>0) and alg=='D+':
                        with record_function("regional_sde"):
                    ...
                    if repeat>1:
                        with torch.no_grad():
                            with record_function("timetravel_unet"):
